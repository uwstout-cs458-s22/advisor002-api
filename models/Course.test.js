...
describe('Course Model', () => {
  ...
  describe('Creating a Course', () => {
    test('Create course with no input parameters', async () => {
      await expect(Course.createCourse()).rejects.toThrowError(
        'Course name, section, credits, and semester are required'
      );
    ...
    test('Create course successfully but no response from database', async () => {
      ...
      await expect(Course.createCourse('fake course', 4, 4)).rejects.toThrowError(
    ...
    test('Create course unsuccessfully due to already existing course', async () => {
      ...
      await expect(Course.createCourse('fake course', 4, 4)).rejects.toThrowError(
        `Course ${row.name} already exists in table "course"`
      );
    });
  ...
  describe('Edit a Course', () => {
    ...
    test('Throw 400 error for no input', async () => {
      ...
      await expect(Course.editCourse(row.id)).rejects.toThrowError(
        'Id and a course attribute required'
      );
    });
    ...
    test('Throw 500 error for other errors', async () => {
      ...
      await expect(Course.editCourse(row.id, putDoc)).rejects.toThrowError(
    ...
    test('Edit a course to have new category prefix', async () => {
      ...
      expect(db.query.mock.calls[0][0]).toBe(`SELECT id FROM category WHERE prefix='CS';`);
    ...
    test('Edit a course to have a new semester', async () => {
      ...
      expect(db.query.mock.calls[1][0]).toBe(`SELECT * FROM "courseSemester" WHERE "courseid"=$1;`);
      expect(db.query.mock.calls[2]).toHaveLength(2);
      expect(db.query.mock.calls[2][0]).toBe(
    ...
    test('Throw 404 for semester not found', async () => {
      ...
      await expect(Course.editCourse(row.id, putDoc)).rejects.toThrowError(
        `Semester with the specifications ${JSON.stringify({
          type: putDoc.type,
          year: putDoc.year,
        })}`
      );
...
describe('querying all courses', () => {
  ...
  test('should make a call to Course.findAll - no criteria, no limits, no offsets', async () => {
    ...
    expect(db.query.mock.calls[0]).toHaveLength(2);
    expect(db.query.mock.calls[0][0]).toBe(
      'SELECT c.*, s.type, s.year from "course" AS c LEFT JOIN "courseSemester" AS cs ON cs.courseId = c.id LEFT JOIN "semester" AS s ON s.id = cs.semesterId  LIMIT $1 OFFSET $2;'
    );
